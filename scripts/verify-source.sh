...
grep -q 'JUICE_BUILD_TREE_PE_READY' "$ROOT/scripts/assemble-runtime.sh"
grep -q 'msvcp140/aarch64-windows/msvcp140.dll' "$ROOT/config/runtime-modules.txt"
grep -q 'dbghelp/aarch64-windows/dbghelp.dll' "$ROOT/config/runtime-modules.txt"
grep -q 'dwmapi/aarch64-windows/dwmapi.dll' "$ROOT/config/runtime-modules.txt"
grep -q 'environmentForExe:' "$ROOT/app/main.m"
grep -q 'WINEPATH=' "$ROOT/app/main.m"
grep -q 'open_jailbreak_library' "$ROOT/launcher/juice-lowva-helper.c"
grep -q 'map_apiset_file' "$ROOT/wine/dlls/ntdll/unix/loader.c"
